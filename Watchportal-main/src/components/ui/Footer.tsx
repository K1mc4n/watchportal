...
import { Home, Newspaper, PlusSquare, MessageCircle, type LucideIcon } from 'lucide-react'; // Shield di hapus
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { ChatWindow } from '~/components/ui/ChatWindow';

// Komponen untuk tombol navigasi biasa
  ...
  <Link href={href} className="flex-1">
    <div
      className={`flex flex-col items-center justify-center p-2 rounded-lg transition-colors ${
        isActive ? 'text-gold' : 'text-gray-500 hover:text-gold'
      }`}
    >
...
// Komponen terpisah untuk tombol Chat
...
export function Footer() {
  ...
  // Hapus 'supporters' dari tipe Tab
  const getActiveTab = (): 'home' | 'news' | 'submit' | 'chat' | null => {
